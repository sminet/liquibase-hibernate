...
public class ColumnSnapshotGenerator extends HibernateSnapshotGenerator {
			...
			if (hibernateColumn.getName().equalsIgnoreCase(column.getName())) {
				...
				if (hibernatePrimaryKey != null) {
					...
					if (isPrimaryKeyColumn) {
						String identifierGeneratorStrategy = hibernateColumn.getValue().isSimpleValue() ? ((SimpleValue) hibernateColumn.getValue()).getIdentifierGeneratorStrategy() : null;
						if (shouldUseAutoIncrement(cfg, identifierGeneratorStrategy)) {
	...
	protected boolean shouldUseAutoIncrement(Configuration cfg, String identifierGeneratorStrategy) {
		...
		// possible custom strategies
		try {
			// test if the class exists
			XClass generator = cfg.getReflectionManager().classForName(identifierGeneratorStrategy);
			XClass identityGenerator = cfg.getReflectionManager().classForName("org.hibernate.id.IdentityGenerator");

			// and has IdentityGenerator as superclass
			if (identityGenerator.isAssignableFrom(generator)) {
				return true;
			}
		} catch (Exception e) {
			// fail
